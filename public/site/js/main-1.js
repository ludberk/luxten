...
const mobilNomreInput = document.querySelector('#mobilNomre');
const sifarisForm = document.querySelector('#sifarisForm');
const mobilNomreError = document.querySelector('#mobilNomreError');

mobilNomreInput?.addEventListener('keydown', (e) => {
...
sifarisForm?.addEventListener('submit', (e) => {
    const mobilNomreInputValue = mobilNomreInput.value.replace(/\s/g, "");
    if(!/^\+994\d{9}$/.test(mobilNomreInputValue)) {
        e.preventDefault();
        mobilNomreError.style.display = 'block';
    }
...
function setMenuStatus(){
    ...
    const headerMenuLinks = headerMenu?.querySelectorAll('a.menu-item-link');
    
    headerMenuLinks?.forEach(headerMenuLink => {
        headerMenuLink.classList.remove('active');
        // console.log(headerMenuLink.attributes.href.value)
    })
    
    const headerMenuLinkActive = Array.from(headerMenuLinks).find((headerMenuLink) => headerMenuLink.attributes.href.value === match[0]);
    headerMenuLinkActive?.classList.add('active');
    console.log(headerMenuLinkActive);
